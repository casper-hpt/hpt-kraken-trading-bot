...
def _resolve_watchlist(cfg: Config, watchlist_path: str) -> list:
    if cfg.crypto_list:
        from .watchlist import WatchItem
        return [WatchItem(symbol=s) for s in cfg.crypto_list]
...
def run_cycle(cfg: Config, watchlist_path: str) -> int:
    ...
    watchlist_items = _resolve_watchlist(cfg, watchlist_path)
