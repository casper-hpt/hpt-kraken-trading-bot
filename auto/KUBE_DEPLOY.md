...
Deploy the full 2Apollo stack (infrastructure + crypto portfolio services + LLM bot) to a k3s cluster.

> **Note:** K8s manifests live under `auto/k8s/`. Infrastructure manifests are in `k8s/utils/`, portfolio service manifests are in `k8s/portfolio/`, and the LLM service is in `k8s/llm/`.
...
# Crypto portfolio
docker build --platform linux/amd64 -t $REGISTRY/crypto-data-collector:latest auto/crypto-data-collector/
docker build --platform linux/amd64 -t $REGISTRY/crypto-trader:latest          auto/crypto-trader/

# LLM service
docker build --platform linux/amd64 -t $REGISTRY/hpt-llm:latest               auto/llm/

# Push all
docker push $REGISTRY/crypto-data-collector:latest
docker push $REGISTRY/crypto-trader:latest
...
**crypto-trader** (Kraken API keys):
```bash
kubectl create secret generic crypto-trader-secret \
  --from-literal=KRAKEN_API_KEY='your-api-key' \
  --from-literal=KRAKEN_API_SECRET='your-api-secret'
```

**hpt-llm** (Slack tokens):
```bash
...
```bash
kubectl apply -f k8s/portfolio/questdb.yaml
...
```bash
kubectl apply -f k8s/utils/prometheus.yaml
...
```bash
kubectl apply -f k8s/utils/grafana.yaml
...
```bash
kubectl apply -f k8s/portfolio/crypto-data-collector.yaml
kubectl rollout status deployment/crypto-data-collector
```

### Crypto Trader

```bash
kubectl apply -f k8s/portfolio/crypto-trader.yaml
kubectl rollout status deployment/crypto-trader
```
...
kubectl rollout status deployment/hpt-llm
```
...
kubectl logs -f deployment/crypto-trader
kubectl logs -f deployment/hpt-llm
```
...
kubectl apply -f k8s/portfolio/<service-name>.yaml
...
k8s/
  portfolio/
    questdb.yaml                          # QuestDB infra
    crypto-data-collector.yaml
    crypto-trader.yaml
  utils/
    prometheus.yaml                       # Prometheus infra
    grafana.yaml                          # Grafana infra
  llm/
    hpt-llm.yaml                          # LLM Slack bot
```
...
|-----------|------|------|-------|---------|
| QuestDB | `k8s/portfolio/questdb.yaml` | StatefulSet | 9000, 8812, 9009 | 10Gi PVC |
| Prometheus | `k8s/utils/prometheus.yaml` | Deployment | 9090 | emptyDir |
| Grafana | `k8s/utils/grafana.yaml` | Deployment | 3000 | none |

### Application Services

| Service | YAML | Metrics Port | Secrets Required |
|---------|------|-------------|-----------------|
| crypto-data-collector | `k8s/portfolio/crypto-data-collector.yaml` | 9092 | None |
| crypto-trader | `k8s/portfolio/crypto-trader.yaml` | 9095 | `KRAKEN_API_KEY`, `KRAKEN_API_SECRET` |
| hpt-llm | `k8s/llm/hpt-llm.yaml` | — | `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN` |
